...
class Mdmessages extends CI_Model{
	...
	function count_records_by_recipient_new($recipient,$utype = 1){
		
		$query = "SELECT COUNT(*) AS cnt FROM messages WHERE ((messages.recipient = $recipient AND messages.system = 0) OR (messages.group = $utype AND messages.system = 1 AND messages.recipient = $recipient) OR (messages.group = $utype AND messages.system = 1 AND messages.recipient = 0)) AND newmail = 1";
		$query = $this->db->query($query);
		$data = $query->result_array();
		if(isset($data[0]['cnt'])) return $data[0]['cnt'];
		return 0;
