					...
					<?php for($i=0;$i<count($tickets);$i++):?>
						<tr <?=($tickets[$i]['status'] && !$hideticket)?'style="display:none;"':''?>>
							...
							<td style="text-align:center; vertical-align:middle;">
								<?=anchor('webmaster-panel/actions/tickets-inbox/read-ticket-id/'.$tickets[$i]['id'],'<i class="icon-comment icon-white"></i>',array('title'=>'Читать тикет','class'=>'btn btn-success'));?>
							<?php if($tickets[$i]['status']):?>
		...
		<?php endif;?>
			$("#ClosedToggle").click(function(){
				$.post("<?=$baseurl;?>webmaster-panel/actions/tickets/hide-closed-tickets",
					{'toggle':true},function(data){window.location="<?=$baseurl;?>webmaster-panel/actions/tickets-inbox"},"json");
			});
