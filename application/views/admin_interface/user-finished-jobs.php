			...
			<div class="span9">
				<ul class="breadcrumb">
					...
					<li style="float:right;">
						<?=anchor('admin-panel/management/finished-jobs/delete/user/'.$this->uri->segment(5),'Удалить все работы',array('class'=>'btn btn-warning','id'=>'DeleteAll','style'=>'margin-top: -5px;'));?>
					</li>
				</ul>
		...
		<?php endif;?>	
			...
			function suggest(inputString){
				if(inputString.length < 5){
					...
					$.post("<?=$baseurl;?>admin-panel/actions/finished-jobs/users-jobs/search",{squery: ""+inputString+"",user:<?=$this->uri->segment(5);?>},
						function(data){
							if(data.status){
								$("#suggestions").fadeIn();
