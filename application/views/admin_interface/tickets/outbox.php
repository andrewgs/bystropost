		...
		<div class="row">
			<div class="span9">
				...
				<div id="frmInsTicket" style="display:none;">
					<?php $this->load->view('forms/adminaddticket');?>
				</div>
				<div class="clear"></div>
				<table class="table table-bordered">
					...
					<?php for($i=0;$i<count($tickets);$i++):?>
						<tr <?=($tickets[$i]['status'] && !$hideticket)?'style="display:none;"':''?>>
							...
							<td>
								<?=anchor('admin-panel/actions/tickets-outbox/read-ticket-id/'.$tickets[$i]['id'],$tickets[$i]['title'],array('title'=>'Читать тикет'));?>
							...
							<td style="text-align:center; vertical-align:middle;">
								<?=anchor('admin-panel/actions/tickets-outbox/read-ticket-id/'.$tickets[$i]['id'],'<i class="icon-comment icon-white"></i>',array('title'=>'Читать тикет','class'=>'btn btn-success'));?>
							<?php if($tickets[$i]['status']):?>
								<div style="height:3px;"> </div>
								<?=anchor('admin-panel/actions/tickets/open-ticket/'.$tickets[$i]['id'],'<img src="'.$baseurl.'images/icons/unlocked.png" alt="" />',array('title'=>'Открыть тикет','class'=>"btn openTicket"));?>
							<?php endif;?>
							</td>
						</tr>
					<?php endfor; ?>
		...
		<?php endif;?>
			$("#ClosedToggle").click(function(){
				$.post("<?=$baseurl;?>admin-panel/actions/tickets/hide-closed-tickets",
					{'toggle':true},function(data){window.location="<?=$baseurl;?>admin-panel/actions/tickets-outbox"},"json");
			});
