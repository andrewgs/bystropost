					...
					<?php for($i=0;$i<count($tickets);$i++):?>
						<tr <?=($tickets[$i]['status'] && !$hideticket)?'style="display:none;"':''?>>
							...
							<?php if($tickets[$i]['status']):?>
								...
								<?=anchor('manager-panel/actions/tickets/open-ticket/'.$tickets[$i]['id'],'<img src="'.$baseurl.'images/icons/unlocked.png" alt="" />',array('title'=>'Открыть тикет','class'=>"btn openTicket"));?>
							<?php endif;?>
							</td>
						</tr>
					<?php endfor; ?>
